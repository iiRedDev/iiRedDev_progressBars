        ...
        window.addEventListener('message', (event) => {
            ...
            if (item !== undefined && item.type === "ui") {
                if (item.display === true) {
                    ...
                    function cercleUP() {
                     /*variables du cercleUP*/
                    let now = new Date();
                    let timeDiff = now.getTime() - start.getTime();
                    let pourcant = Math.round((timeDiff / maxTime) * 100);
                    let pourcant1 = pourcant / 100;

                        console.log(pourcant);         
                        if (pourcant <= 100) {
                            $('#cercle').circleProgress({
                                animation: false,
                                value: pourcant1,
                                
                            });
                            $('#pourcent').html("<strong>" + pourcant + '<i>%</i></strong>');
                            
                            setTimeout(cercleUP, timeoutVal);
                        } else {
                            $("#iiDark").hide();
                        }
                    }
